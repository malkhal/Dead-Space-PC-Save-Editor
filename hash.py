...
import sys
from common import *
from validatesavefile import MC02Header

def calclateHash(hexlist, dataOffset, sizeOffset = 0):
    saltlist = [0x00000000, 0x04C11DB7, 0x09823B6E, 0x0D4326D9, 0x130476DC, 0x17C56B6B, 0x1A864DB2, 0x1E475005, 0x2608EDB8, 0x22C9F00F,
    ...
    hexint = [int(x, 16) for x in hexlist]
    dataLen = 0x18
    if (sizeOffset != 0):
        dataLen = readInt32(hexlist,sizeOffset)
    mhash = ~(hexint[dataOffset] << 0x18 | hexint[dataOffset+1] << 0x10 | hexint[dataOffset+2] << 0x8 | hexint[dataOffset+3]) + 2**32
    for x in range(0,dataLen-4):
        mhash = (((mhash & 0x00FFFFFF) << 8 | hexint[dataOffset + 4 + x]) ^ saltlist[mhash >> 0x18])
    return (~mhash + 2**32 )& 0xffffffff

def hashData(hexlist):
    hash0 = calclateHash(hexlist,0x2850,0x283C)
    writeInt32(hash0,hexlist,0x2844)
    hash1 = calclateHash(hexlist,0x2CC8,0x2840)
    writeInt32(hash1,hexlist,0x2848)
    hash2 = calclateHash(hexlist,0x2834,0)
    writeInt32(hash2,hexlist,0x284C)
